...
零碳产业子类示例：物质循环(资源处理/有机物碳循环/无机物)、能量循环(发电/储能/氢基能源)
AI与智能科技子类：AI软件层(底座大模型/工程改进)、AI硬件层(半导体/芯片/计算集群/数据中心)、具身智能、量子信息
通用技术：检测表征、通信运输(电网/航天/航空/陆路/水路)、催化剂、材料工程

只输出JSON数组，每个元素：{"id":0,"category":"完整路径或不相关","tag":"news专属标签或空","topic":"5字名词性短语"}
...
def call_glm(prompt, batch_json, timeout=TIMEOUT):
    ...
    try:
        result = subprocess.run(
            ["hermes", "-z", full_prompt, "--provider", "zai", "-m", "glm-5.3-flash", "--cli"],
...
def main():
    with open("/tmp/unscored_records.json") as f:
        records = json.load(f)
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                batch_start_id, result = future.result()
                if result:
                    for item in result:
            ...
            if done % 10 == 0:
                print(f"  {done}/{len(batches)} batches classified")
    ...
    def score_batch(batch):
        return (batch[0]["id"], call_glm(SCORE_PROMPT, batch, timeout=TIMEOUT))
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for future in as_completed(futures):
            ...
            if done % 10 == 0:
                print(f"  {done}/{len(batches)} batches scored")
    
    print(f"Scored: {len(score_results)}/{len(records)}")
    
    # Merge results
    DOMAIN_THRESHOLDS = {"零碳产业": 6.5, "AI与智能科技": 6.5, "通用技术": 7.0}
    ...
    for i, r in enumerate(records):
        ...
        if category != "不相关" and score_item:
            ...
            score = round(b*WEIGHTS["b"] + ind*WEIGHTS["i"] + rar*WEIGHTS["r"] + dat*WEIGHTS["d"] + tim*WEIGHTS["t"], 1)
            scd = {"b": b, "i": ind, "r": rar, "d": dat, "t": tim}
            
            domain = category.split("-")[0] if "-" in category else category.split("/")[0]
            threshold = DOMAIN_THRESHOLDS.get(domain, 7.0)
            if score >= threshold:
                aip = 1
    ...
    with open("/tmp/new_classified_scored.json", "w") as f:
    ...
    print(f"Classified (non-不相关): {sum(1 for m in merged if m['category'] != '不相关')}")
    print(f"Scored: {scored_count}")
    print(f"AI精选: {aip_count}")
    print(f"Scores: {sorted([m['score'] for m in merged if m['score']>0], reverse=True)[:20]}")

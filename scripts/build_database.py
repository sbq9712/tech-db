...
SOURCE_REPOS = {
    'wechat-daily-news-csv': {'kind': 'news', 'subdir': 'csv', 'patterns': [re.compile(r'^\d{4}-\d{2}-\d{2}\.csv$')], 'title_keys': ['标题', 'title'], 'body_keys': ['clean_text', 'content_preview', '正文', 'content', 'summary', '摘要'], 'author_keys': ['account_name', '作者', 'author'], 'url_keys': ['URL', 'url', 'link']},
    'news-spider': {'kind': 'news', 'subdir': 'data', 'patterns': [re.compile(r'^articles-\d{4}-\d{2}-\d{2}\.csv$')], 'title_keys': ['title', '标题'], 'body_keys': ['content', '正文', 'summary', '摘要'], 'author_keys': ['source_name', 'domain', 'author', '作者'], 'url_keys': ['url', 'URL', 'link']},
    'literature-rss-spider': {'kind': 'literature', 'subdir': 'output', 'patterns': [re.compile(r'^news_with_abstract_\d{4}-\d{2}-\d{2}\.csv$')], 'title_keys': ['title', '标题'], 'body_keys': ['abstract', 'summary', '摘要', 'content'], 'author_keys': ['source', 'author', 'authors', '作者'], 'url_keys': ['link', 'url', 'URL']},
}
...
def load_existing_classifications(processed: Path) -> dict[str, dict[str, Any]]:
    """Load existing classifications AND tags/topics/params from processed shards.
    
    CRITICAL: This preserves tag/topic/key_params/intelligence_type across rebuilds.
    Without this, GitHub Actions rebuilds would wipe all LLM-generated tags every time.
    """
    ...
    for shard in processed.glob('records-*.json'):
        ...
        for record in payload.get('records', []):
            ...
            if record.get('category') and record.get('category') != '未分类':
                preserved['category'] = record.get('category')
                preserved['classification_confidence'] = record.get('classification_confidence', 0.0)
                preserved['classification_reason'] = record.get('classification_reason', '')
                preserved['classifier_model'] = record.get('classifier_model')
                preserved['classifier_provider'] = record.get('classifier_provider')
            if record.get('tag'):
                preserved['tag'] = record.get('tag')
            if record.get('topic'):
                preserved['topic'] = record.get('topic')
            if 'key_params' in record:
                preserved['key_params'] = record.get('key_params')
            if record.get('intelligence_type') and record.get('source') and ('news-spider' in record.get('source', '') or 'wechat' in record.get('source', '')):
                preserved['intelligence_type'] = record.get('intelligence_type')
            preserved['key_parameters'] = []
            preserved['cache_hit'] = False
            if preserved:

...
function render() {
  ...
  $('list').innerHTML = filtered.slice(0, 120).map((item) => `
    <div class="item ${item.content_hash === state.selectedId ? 'active' : ''}" data-id="${item.content_hash}">
      <div class="item-top">
        <div>
          <div class="item-title">${item.title || '未命名情报'}</div>
          <div class="item-meta">${item.date || '未知日期'} · ${item.source || '未知来源'} · ${item.intelligence_type === 'literature' ? '文献' : '新闻'}</div>
        </div>
...
document.addEventListener('click', (event) => {
  ...
  if (categoryLink) { event.preventDefault(); state.query = categoryLink.dataset.cat; $('search').value = state.query; render(); return; }
  const item = event.target.closest('[data-id]');
  if (item) { state.selectedId = item.dataset.id; render(); }
});
